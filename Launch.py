...
def MasterData(inppath,outpth,paidamount_file,tax_data):
    ...
    zonemap, usemap, construcmap, occpmap, subusemap, \
        gatnamemap, splownmap, splaccmap, wrong_pid = \
        read_data.mapping_data(mappath)
             ...
             property_receipt_df, \
                         shasti_flags, \
                            bill_distributed_details,\
                                        japtinotice_data = read_data.execute_data(inppath,tax_data,wrong_pid)

    # Process Property details list.
    ...
    tax_process.data_process(tday_dmyfmt,mappath,outpth, property_data, property_list_df, last_receiptdate_pkey,
                               japti_flagger,shasti_flags,bill_distributed_details,paidamount_ly,paidamount_ty)

# Start
if __name__ == '__main__':
    directory = confgfile['main_path']
    std_path = directory + "Master Data/"
    inppath = std_path + "Input/"
    outpth = std_path + "Output/" + tday_dbyfmt + "/"
    paidamount_file = directory + "Paidamount/"
    tax_data = directory + "/Tax_Data/"
